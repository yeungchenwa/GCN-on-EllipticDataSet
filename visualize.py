...
from sklearn.metrics import auc, confusion_matrix, average_precision_score, precision_recall_curve
...
parser.add_argument('--checkpoint_path', default='D:/checkpoint_BTC_abnormal_trans/md-gcn_order3/best_29_total_0.831039_0.613112_0.705632.pth', help='Checkpoint saved path')
...
test_recall_ts = []
test_f1_ts = []
test_ts = range(args.end_ts-args.max_train_ts)
...
# roc_curve
fpr, tpr, _ = metrics.roc_curve(test_label_all, test_pred_all)
plt.plot(fpr, tpr, label='ROC')
plt.xlabel('False Positive Rate')
plt.ylabel('True Ppsitive Rate')
plt.show()

# precision-recal curve
precision, recall, thresholds = precision_recall_curve(test_label_all, test_pred_all)
...
plt.show()
# compute AP
AP = average_precision_score(test_label_all, test_pred_all, average='micro')
print('AP:', AP)

# confusion matrix
confusion_mat = confusion_matrix(test_label_all, test_pred_all)
fig = plt.figure()
